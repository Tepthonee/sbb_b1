...
async def upstream(event):
    ...
    if ENV and (HEROKU_API_KEY is None or HEROKU_APP_NAME is None):
        return await edit_or_reply(
            event, "⌔∮ يجب عليك وضع الفارات المطلوبة لتحديث لسورس تيبثون"
        )
    try:
    ...
    if changelog == "" and not force_update:
        await event.edit("\n⌔∮ عزيز المستخدم انت تستخدم اخر اصدار من بوت تيبثون 🫂♥")
        return repo.__del__()
    ...
    if conf == "الان":
        await event.edit("⌔∮ جارِ تحديث بوت تيبثون يرجى الأنتظار قليلا")
